...
export const CronTime = () => {
  ...
  const handleChange = async ({ id, name, schedule, status }: { id: number; name?: string; schedule?: string; status?: boolean }) => {
    try {

      console.log('Updating cron job:', { id, name, schedule, status });
      const body = { id, ...(name !== undefined && { name }), ...(schedule !== undefined && { schedule }), ...(status !== undefined && { status }) };
